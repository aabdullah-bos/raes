...
- your final answer must end with exactly one tagged JSON block
- the block must begin with `RAES_SUMMARY_START` on its own line
- the block must end with `RAES_SUMMARY_END` on its own line
- the content between those markers must be valid JSON
- use empty arrays (`[]`) for sections with no entries
- do not include markdown inside the JSON block
- if you cite a file, include `path` and `line` when available
...
Required JSON shape:
{
  "slice": {
    ...
    "pipeline": { "path": "string", "line": 1 },
    "position": "string | number"
  },
  "artifactsInspected": [{ "path": "string", "line": 1 }],
  "repoInspection": [{ "text": "string", "refs": [{ "path": "string", "line": 1 }] }],
  "plan": ["string"],
  "testsAddedOrUpdated": [{ "text": "string", "refs": [{ "path": "string", "line": 1 }] }],
  "implementationChanges": [{ "text": "string", "refs": [{ "path": "string", "line": 1 }] }],
  "findings": [{ "text": "string", "refs": [{ "path": "string", "line": 1 }] }],
  "validation": ["string"],
  "gaps": ["string"],
